...
export default function AddExpense() {
  ...
  // Auto-focus description input on mount using useRef
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      ...
      const res = await fetch(`${baseURL}/group-memberships/group/${groupId}`, {
        method: 'GET',
        headers: { 
      ...
      const memberList = data.map((m) => m.userId);
  ...
  const handleSubmit = async (e) => {
    ...
    const settlementRes = await fetch(`${baseURL}/settlements/${groupId}`, {
      method: 'POST',
      headers: { 
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!settlementRes.ok) {
      const errorData = await settlementRes.json();
      throw new Error(errorData.message || 'Failed to update settlements');
    }
